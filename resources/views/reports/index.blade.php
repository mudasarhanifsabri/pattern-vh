    ...
    <section class="grid gap-3 md:grid-cols-4">
        @foreach($cards as $card)
            <article class="erp-card p-5">
                ...
                <p class="mt-3 text-2xl font-black {{ $card['value'] < 0 ? 'text-rose-600' : 'text-[#071a3b]' }}">AED {{ number_format((float) $card['value'], 2) }}</p>
                <p class="mt-1 text-xs text-slate-500">{{ $card['note'] }}</p>
                @can('reports.export')
                    <a href="{{ route('reports.export', ['type' => $card['type'], 'from' => $from->format('Y-m-d'), 'to' => $to->format('Y-m-d')]) }}" class="mt-3 inline-flex text-xs font-black text-blue-600">Export CSV</a>
                @endcan
            </article>
        @endforeach
    </section>
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
            ...
            <div class="flex flex-wrap gap-2">
                @foreach(['bookings' => 'Bookings', 'invoices' => 'Invoices', 'payments' => 'Payments', 'expenses' => 'Expenses'] as $type => $label)
                    <a href="{{ route('reports.export', ['type' => $type, 'from' => $from->format('Y-m-d'), 'to' => $to->format('Y-m-d')]) }}" class="rounded-xl border border-slate-200 px-4 py-2 text-xs font-black text-slate-600 hover:bg-slate-50">{{ $label }} CSV</a>
                @endforeach
            </div>
        </div>
    </section>
