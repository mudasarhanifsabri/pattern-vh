        ...
        <section class="erp-card p-5">
            ...
            <div class="mt-5 space-y-3 md:hidden">
                @forelse($refunds as $refund)
                    @php($depositReceipt = $refund->booking->depositReceiptRecord)
                    <div class="rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-sm">
                        <a href="{{ route('bookings.show', $refund->booking) }}" class="block">
                            <div class="flex items-start justify-between gap-3"><div><p class="text-xs font-bold uppercase tracking-[0.16em] text-blue-600">{{ $refund->booking->booking_no }}</p><h3 class="mt-1 text-lg font-black text-[#071a3b]">{{ $refund->tenant->full_name }}</h3><p class="mt-1 text-sm text-slate-500">{{ $refund->booking->unit->building->name }} / Unit {{ $refund->booking->unit->unit_no }}</p></div><span class="rounded-full px-2.5 py-1 text-[11px] font-bold {{ $statusClasses[$refund->status] ?? 'bg-slate-50 text-slate-600' }}">{{ str($refund->status)->replace('_', ' ')->headline() }}</span></div><div class="mt-4 grid grid-cols-3 gap-2 text-center text-xs"><div class="rounded-2xl bg-slate-50 p-3"><p class="font-bold text-slate-400">Deposit</p><p class="mt-1 font-black text-[#071a3b]">{{ number_format((float) $refund->deposit_amount, 0) }}</p></div><div class="rounded-2xl bg-rose-50 p-3"><p class="font-bold text-rose-400">Damage</p><p class="mt-1 font-black text-rose-700">{{ number_format((float) $refund->damage_amount, 0) }}</p></div><div class="rounded-2xl bg-emerald-50 p-3"><p class="font-bold text-emerald-500">Refund</p><p class="mt-1 font-black text-emerald-700">{{ number_format((float) $refund->refund_amount, 0) }}</p></div></div>
                        </a>
                        <div class="mt-4">
                            @if($depositReceipt)
                                <a href="{{ route('receipts.pdf', $depositReceipt) }}" target="_blank" class="inline-flex h-10 w-full items-center justify-center rounded-xl bg-emerald-600 px-3 text-xs font-black text-white">Deposit receipt</a>
                            @else
                                <span class="inline-flex h-10 w-full items-center justify-center rounded-xl bg-amber-50 px-3 text-xs font-black text-amber-700">Receipt pending</span>
                            @endif
                        </div>
                    </div>
                @empty
        ...
        <aside class="erp-card p-5">
            ...
            <div class="mt-4 space-y-3">
                @forelse($activeBookings as $booking)
                    @php($depositReceipt = $booking->depositReceiptRecord)
                    <div class="rounded-2xl border border-slate-200 p-4 hover:bg-slate-50"><a href="{{ route('bookings.show', $booking) }}" class="block"><div class="flex items-center justify-between gap-3"><div><p class="font-bold text-[#071a3b]">{{ $booking->booking_no }}</p><p class="mt-1 text-xs text-slate-500">{{ $booking->tenant->full_name }} / {{ $booking->unit->building->name }} {{ $booking->unit->unit_no }}</p></div><span class="font-black text-[#071a3b]">AED {{ number_format((float) $booking->deposit_amount, 2) }}</span></div><p class="mt-2 text-xs text-slate-500">Refund workflow: {{ $booking->depositRefund ? str($booking->depositRefund->status)->replace('_', ' ')->headline() : 'Not started' }}</p></a>@if($depositReceipt)<a href="{{ route('receipts.pdf', $depositReceipt) }}" target="_blank" class="mt-3 inline-flex rounded-xl bg-emerald-600 px-3 py-2 text-xs font-black text-white">Deposit receipt</a>@else<span class="mt-3 inline-flex rounded-xl bg-amber-50 px-3 py-2 text-xs font-black text-amber-700">Receipt pending</span>@endif</div>
                @empty
