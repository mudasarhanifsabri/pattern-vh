            ...
            <div class="space-y-5">
                <div class="erp-card p-5" data-record-panel="profile">
                    ...
                    <dl class="mt-6 grid gap-4 md:grid-cols-2">
                        ...
                        <div class="rounded-2xl bg-slate-50 p-4"><dt class="text-xs font-bold uppercase tracking-[0.16em] text-slate-400">Identity</dt><dd class="mt-1 font-bold text-[#071a3b]">{{ str($record->identity_type)->replace('_', ' ')->headline() }} {{ $record->identity_no }}</dd><dd class="mt-1 text-xs text-slate-500">Expires {{ $record->identity_expiry_date?->format('M d, Y') ?? 'Not set' }}</dd></div>
                        <div class="rounded-2xl bg-slate-50 p-4"><dt class="text-xs font-bold uppercase tracking-[0.16em] text-slate-400">Date of birth</dt><dd class="mt-1 font-bold text-[#071a3b]">{{ $record->date_of_birth?->format('M d, Y') ?? 'Not added' }}</dd></div>
                    </dl>
