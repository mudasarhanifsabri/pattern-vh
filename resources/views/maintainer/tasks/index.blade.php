<x-dynamic-component component="layouts.maintainer-pwa">
    <div class="purple-head px-6 pb-7 pt-7 text-white">
        <div class="flex items-center justify-between">
            <a href="{{ route('maintainer.profile') }}" class="text-sm font-black">Menu</a>
            <a href="{{ route('maintainer.notifications') }}" class="relative text-sm font-black">Alerts<span class="absolute -right-3 -top-3 rounded-full bg-red-500 px-1.5 text-[10px]">{{ $stats['total'] }}</span></a>
        </div>
    ...
    <main class="app-scroll px-5 pt-5">
        ...
        <form method="GET" action="{{ route('maintainer.tasks.index') }}" class="mb-4 flex gap-2">
            ...
            <button class="grid h-12 w-12 place-items-center rounded-xl border border-slate-200 bg-white text-xs font-black">Go</button>
        </form>
