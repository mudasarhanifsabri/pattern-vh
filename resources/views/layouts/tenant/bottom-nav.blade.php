<nav class="{{ request()->routeIs('support.*') ? 'hidden' : 'grid' }} fixed inset-x-0 bottom-0 z-40 mx-auto max-w-[430px] grid-cols-4 border-t border-slate-100 bg-white/95 px-3 pb-4 pt-2 shadow-[0_-12px_30px_rgba(15,23,42,0.08)] backdrop-blur">
    @foreach ([
        ...
        <a href="{{ route($tab['route']) }}" class="flex flex-col items-center gap-1 px-1 py-2 text-[11px] font-bold {{ request()->routeIs($tab['route']) || ($tab['route'] === 'bookings.index' && request()->routeIs('bookings.*')) || ($tab['route'] === 'support.index' && request()->routeIs('support.*')) ? 'text-blue-600' : 'text-slate-500' }}">
